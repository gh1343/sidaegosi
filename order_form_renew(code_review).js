$(document).ready(function(){
	...
	$('.order-item.order-item-sale .order-item-title i').click(function(){
		if($(this).hasClass("on")) {
			...
			if( parseInt(uncomma($('.tt_movie').text())) != 0 ){//0원이 아닌경우 노출(값이 들어가면 노출되어야함)
...
$('.ois-btn-more1').each(function(eq){
	...
	$("#dsp_cash").on("propertychange keyup input click", function(){
		if( $('.cash_kind:checked').length == 0 ){
			alert('상품을 먼저 선택해주세요.');
		}
		var cash_put = numchk($(this).val());
		$('.final_cash').text(cash_put);
		//$('.final_cash').text(cash_put);
	});
});
